function showPurchasePrompt() {
  ...
  function displayInvoice(items) {
    ...
    const summaryRows = [
      ...
      ["Shipping to", getShippingCost(itemTotal)],
      ["Subtotal", "$" + (itemTotal + getShippingCost(itemTotal)).toFixed(2)],
      ["Tax", "$" + calculateTax(itemTotal + getShippingCost(itemTotal)).toFixed(2)],
      ["Invoice Total", "$" + (itemTotal + getShippingCost(itemTotal) + calculateTax(itemTotal + getShippingCost(itemTotal))).toFixed(2)]
    ];
